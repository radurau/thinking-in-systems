...
NEAR_HALF_W = 322                # half width of the near touchline (centre x = 320)
FAR_SHRINK = 0.32                # far touchline is (1-FAR_SHRINK) as wide as the near one
PIN = 32                         # pin height
GOAL_H, HOOP_H = 34, 50
...
opponents = [(320,92),(335,296),(545,118),(560,288)]          # the other team — rose, unnamed, off the passing lanes
def figure(i, x, y, col, col2):
    gx, gy = P(x, y)
    ...
    body = f'''<ellipse cx="{f(gx)}" cy="{f(gy)}" rx="8" ry="2.8" fill="rgba(0,0,0,.45)"/>
            <circle cx="{f(gx)}" cy="{f(gy-27)}" r="4.6" fill="{col}"/>
            <line x1="{f(gx)}" y1="{f(gy-22)}" x2="{f(gx)}" y2="{f(gy-13)}" stroke="{col}" stroke-width="2.2" stroke-linecap="round"/>
            <line x1="{f(gx)}" y1="{f(gy-13)}" x2="{f(gx-4.5)}" y2="{f(gy)}" stroke="{col}" stroke-width="2.2" stroke-linecap="round"/>'''
    foot = f'''        <g class="pop" style="--d:{0.5+0.07*i:.2f}s">
          <g>{sway}
            {body}
            <g>{kick}<line x1="{f(gx)}" y1="{f(gy-13)}" x2="{f(gx+4.5)}" y2="{f(gy)}" stroke="{col2}" stroke-width="2.2" stroke-linecap="round"/></g>
            <line x1="{f(gx)}" y1="{f(gy-21)}" x2="{f(gx-6)}" y2="{f(gy-13)}" stroke="{col}" stroke-width="2" stroke-linecap="round"/>
            <line x1="{f(gx)}" y1="{f(gy-21)}" x2="{f(gx+6)}" y2="{f(gy-13)}" stroke="{col}" stroke-width="2" stroke-linecap="round"/>
          </g>
        </g>'''
    hand = f'''        <g class="pop" style="--d:{0.5+0.07*i:.2f}s">
          <g>{sway}
            {body}
            <line x1="{f(gx)}" y1="{f(gy-13)}" x2="{f(gx+4.5)}" y2="{f(gy)}" stroke="{col}" stroke-width="2.2" stroke-linecap="round"/>
            <g>{drib}<line x1="{f(gx)}" y1="{f(gy-21)}" x2="{f(gx-6)}" y2="{f(gy-31)}" stroke="{col}" stroke-width="2" stroke-linecap="round"/>
            <line x1="{f(gx)}" y1="{f(gy-21)}" x2="{f(gx+6)}" y2="{f(gy-31)}" stroke="{col}" stroke-width="2" stroke-linecap="round"/></g>
          </g>
        </g>'''
    return gx, gy, foot, hand
for i, (x, y, a, b) in enumerate(players):
    gx, gy, foot, hand = figure(i, x, y, '#7aa2ff', '#9db9ff')
    pinsF.append(foot); pinsH.append(hand)
...
for j, (x, y) in enumerate(opponents):
    gx, gy, foot, hand = figure(len(players)+j, x, y, '#fb7185', '#fda4af')
    pinsF.append(foot); pinsH.append(hand)
cx_far = P(320,50); cx_near = P(320,330)
svg = f'''    <svg class="fig sysfig" viewBox="0 0 680 384" aria-hidden="true" style="margin-top:.6vh;max-width:min(740px,54vw)">
      ...
      <!-- the players — seven in blue, four opponents in rose; feet in football, hands in basketball -->
...
i = s.index("5 · MEADOWS' TEST ====="); j = s.index('6 · STOCKS & FLOWS =====')
